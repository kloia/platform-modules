locals {
  ...
  log_group_arn          = var.lambda_at_edge ? format("arn:%s:%s:%s:%s:%s", data.aws_arn.log_group_arn[0].partition, data.aws_arn.log_group_arn[0].service, "*", data.aws_arn.log_group_arn[0].account, data.aws_arn.log_group_arn[0].resource) : local.log_group_arn_regional
...
resource "aws_iam_policy" "logs" {
  ...
  name   = local.create_role ? "${local.role_name}-logs-${random_string.log_s3_name.result}" : "${data.aws_iam_role.lambda[0].name}-logs-${random_string.log_s3_name.result}"
...
resource "aws_iam_role_policy_attachment" "logs" {
  ...
  role       = local.create_role ? aws_iam_role.lambda[0].name : data.aws_iam_role.lambda[0].name
...
data "aws_iam_policy_document" "dead_letter" {
  count = var.attach_dead_letter_policy ? 1 : 0
...
resource "aws_iam_policy" "dead_letter" {
  ...
  name   = local.create_role ? "${local.role_name}-dl-${random_string.log_s3_name.result}" : "${data.aws_iam_role.lambda[0].name}-dl-${random_string.log_s3_name.result}"
  path   = var.policy_path
...
resource "aws_iam_policy" "vpc" {
  ...
  name   = local.create_role ? "${local.role_name}-vpc-${random_string.log_s3_name.result}" : "${data.aws_iam_role.lambda[0].name}-vpc-${random_string.log_s3_name.result}"
  path   = var.policy_path
...
data "aws_iam_policy" "tracing" {
  count = var.attach_tracing_policy ? 1 : 0
...
resource "aws_iam_policy" "tracing" {
  ...
  name   = local.create_role ? "${local.role_name}-tracing-${random_string.log_s3_name.result}" : "${data.aws_iam_role.lambda[0].name}-tracing-${random_string.log_s3_name.result}"
  path   = var.policy_path
...
data "aws_iam_policy_document" "async" {
  count = var.attach_async_event_policy ? 1 : 0
...
resource "aws_iam_policy" "async" {
  ...
  name   = local.create_role ? "${local.role_name}-async-${random_string.log_s3_name.result}" : "${data.aws_iam_role.lambda[0].name}-async-${random_string.log_s3_name.result}"
  path   = var.policy_path
...
resource "aws_iam_policy" "additional_jsons" {
  ...
  name   = local.create_role ? "${local.role_name}-${count.index}-${random_string.log_s3_name.result}" : "${data.aws_iam_role.lambda[0].name}-${count.index}-${random_string.log_s3_name.result}"
  path   = var.policy_path
...
resource "aws_iam_role_policy_attachment" "additional_many" {
  count = var.attach_policies ? var.number_of_policies : 0
...
resource "aws_iam_policy" "additional_inline" {
  ...
  name   = "${local.role_name}-inline-${random_string.log_s3_name.result}"
  path   = var.policy_path
