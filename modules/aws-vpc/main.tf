...
resource "aws_route_table" "private" {
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.private_subnet_suffix}" : "${var.name}-${var.private_subnets[count.index].name}-${var.private_subnets[count.index].availability_zone}"
    },
    var.tags,
...
resource "aws_route" "private_ipv6_egress" {
  count = local.create_vpc && var.create_private_subnet_route_table && var.create_egress_only_igw && var.enable_ipv6 ? length(var.private_subnets) : 0
...
resource "aws_subnet" "private" {
  ...
  tags = merge(
    {
      "Name" = "${var.private_subnets[count.index].name}-${var.private_subnets[count.index].availability_zone}",
