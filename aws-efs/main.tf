...
data "aws_subnets" "private_subnets_with_tag" {
  filter {
    name   = "vpc-id"
    values = [var.vpc_id]
  }
...
data "aws_subnet" "cidr_blocks" {
  count = length(data.aws_subnets.private_subnets_with_tag.ids)
  id = data.aws_subnets.private_subnets_with_tag.ids[count.index]
}
...
resource "aws_efs_mount_target" "this" {
  for_each = { for k, v in local.mount_targets : k => v if var.create }
...
locals {
  ...
  mount_targets = { for k, v in zipmap(var.azs, data.aws_subnets.private_subnets_with_tag.ids) : k => { subnet_id = v } }
