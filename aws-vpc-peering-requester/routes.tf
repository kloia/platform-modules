...
locals {
    private_route_table_ids = var.create_cross_account_peering ? try(jsondecode(data.aws_ssm_parameter.requester_private_route_table_ids[0].value),null) : null

    public_route_table_ids = var.create_cross_account_peering ?  try(jsondecode(data.aws_ssm_parameter.requester_public_route_table_ids[0].value),null) : null
}
...
resource "aws_route" "cross_account_accepter_private_route_table_route" {
    ...
    destination_cidr_block = try(data.aws_ssm_parameter.requester_vpc_cidr_block[0].value,null)
